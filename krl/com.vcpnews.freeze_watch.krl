...
>>
  ...
  rule convertState {
    select when com_vcpnews_freeze_watch factory_reset
    pre {
      lowestShed = ent:freezing_temps.filter(function(v){v=="Shed"})
                                    .sort(by_num("temp"))
                                    .head()
      lowestPorch = ent:freezing_temps.filter(function(v){v=="Porch"})
                                     .sort(by_num("temp"))
                                     .head()
      latestShed = ent:freezing_temps.filter(function(v){v=="Shed"})
                                    .sort(by("time"))
                                    .reverse()
                                    .head()
      latestPorch = ent:freezing_temps.filter(function(v){v=="Porch"})
                                     .sort(by("time"))
                                     .reverse()
                                     .head()
    }
